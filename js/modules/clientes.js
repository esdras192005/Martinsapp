/* ======================================================================
   ...
   - Cadastrar, editar e excluir veículos vinculados a um cliente
     (usa VeiculosDB) — placa, marca, modelo, ano, cor e km são todos
     opcionais, o cliente pode ir completando aos poucos.
...
const ClientesModule = (() => {
  ...
  function descreverVeiculo(veiculo) {
    ...
    return partes ? `${placa} — ${partes}` : placa;
  }
  ...
  /* --------------------------------------------------------------------
     Modal: formulário de criar/editar veículo do cliente

     Nenhum campo é obrigatório aqui — placa, marca, modelo, ano, cor e
     km podem ficar em branco. A ideia é permitir cadastrar o carro aos
     poucos, completando os dados conforme forem disponíveis.
     -------------------------------------------------------------------- */
  function abrirFormularioVeiculo(clienteId, veiculo = null) {
    const editando = Boolean(veiculo);

    els.modalCorpo.innerHTML = `
      <form id="veiculo-form" novalidate>
        <div class="form-group">
          <label for="campo-placa">Placa <span class="form-optional">(opcional)</span></label>
          <input type="text" id="campo-placa" maxlength="8" placeholder="Ex: ABC1D23" autocomplete="off" style="text-transform:uppercase">
        </div>
        <div class="form-group">
          <label for="campo-marca">Marca <span class="form-optional">(opcional)</span></label>
          <input type="text" id="campo-marca" maxlength="40" placeholder="Ex: Fiat" autocomplete="off">
        </div>
        <div class="form-group">
          <label for="campo-modelo">Modelo <span class="form-optional">(opcional)</span></label>
          <input type="text" id="campo-modelo" maxlength="40" placeholder="Ex: Palio" autocomplete="off">
        </div>
        <div class="form-group">
          <label for="campo-ano">Ano <span class="form-optional">(opcional)</span></label>
          <input type="number" id="campo-ano" min="1950" max="2100" placeholder="Ex: 2018" inputmode="numeric" autocomplete="off">
        </div>
        <div class="form-group">
          <label for="campo-cor">Cor <span class="form-optional">(opcional)</span></label>
          <input type="text" id="campo-cor" maxlength="30" placeholder="Ex: Prata" autocomplete="off">
        </div>
        <div class="form-group">
          <label for="campo-km">Quilometragem <span class="form-optional">(opcional)</span></label>
          <input type="number" id="campo-km" min="0" placeholder="Ex: 45000" inputmode="numeric" autocomplete="off">
        </div>
        <div class="form-group">
          <label for="campo-veiculo-observacoes">Observações <span class="form-optional">(opcional)</span></label>
          <textarea id="campo-veiculo-observacoes" rows="3" maxlength="300" placeholder="Detalhes, avarias, preferências..."></textarea>
        </div>
        <p class="form-erro" id="form-erro-veiculo" hidden></p>
      </form>
    `;

    els.modalRodape.innerHTML = `
      <button type="button" class="btn btn-secondary" id="btn-cancelar-form-veiculo">Cancelar</button>
      ${editando ? `<button type="button" class="btn btn-danger" id="btn-excluir-veiculo">Excluir</button>` : ''}
      <button type="submit" form="veiculo-form" class="btn btn-primary" id="btn-salvar-form-veiculo">Salvar</button>
    `;
    ...
    const campoVeiculoObservacoes = document.getElementById('campo-veiculo-observacoes');
    const erroEl = document.getElementById('form-erro-veiculo');
    ...
    if (editando) {
      ...
      campoKm.value = veiculo.km ?? '';
      campoVeiculoObservacoes.value = veiculo.observacoes || '';
    }

    document.getElementById('btn-cancelar-form-veiculo').addEventListener('click', () => {
      fecharModal();
      abrirDetalhe(clienteId);
    });

    if (editando) {
      document.getElementById('btn-excluir-veiculo').addEventListener('click', async () => {
        const botao = document.getElementById('btn-excluir-veiculo');
        botao.disabled = true;
        try {
          await VeiculosDB.excluir(veiculo.id);
          mostrarToast('Veículo excluído.');
          fecharModal();
          abrirDetalhe(clienteId);
        } catch (erro) {
          mostrarToast(erro.message || 'Não foi possível excluir o veículo.', 'erro');
    ...
    document.getElementById('veiculo-form').addEventListener('submit', async (e) => {
      ...
      const dados = {
        ...
        km: campoKm.value ? Number(campoKm.value) : null,
        observacoes: campoVeiculoObservacoes.value.trim(),
      };

      const botaoSalvar = document.getElementById('btn-salvar-form-veiculo');
      botaoSalvar.disabled = true;

      try {
        if (editando) {
          await VeiculosDB.atualizar(veiculo.id, dados);
          mostrarToast('Veículo atualizado com sucesso.');
        } else {
          await VeiculosDB.criar(dados);
          mostrarToast('Veículo cadastrado com sucesso.');
        }
        fecharModal();
        abrirDetalhe(clienteId);
      } catch (erro) {
        erroEl.textContent = erro.message || 'Não foi possível salvar o veículo.';
        erroEl.hidden = false;
        botaoSalvar.disabled = false;
      }
    });

    abrirModal(editando ? 'Editar Veículo' : 'Novo Veículo');
    campoPlaca.focus();
  }
  ...
  async function abrirDetalhe(id) {
    ...
    const linhaVeiculos = veiculos.length
      ? `<ul class="detalhe-veiculos-lista">
          ${veiculos.map((v) => `
            <li class="detalhe-veiculo-item" data-veiculo-id="${v.id}" role="button" tabindex="0">
              <span>${escapeHtml(v.placa || 'Sem placa')}${v.marca || v.modelo ? ` — ${escapeHtml([v.marca, v.modelo].filter(Boolean).join(' '))}` : ''}</span>
              ${v.cor ? `<span class="detalhe-veiculo-extra">${escapeHtml(v.cor)}</span>` : ''}
              ${v.km != null ? `<span class="detalhe-veiculo-extra">${Number(v.km).toLocaleString('pt-BR')} km</span>` : ''}
            </li>
          `).join('')}
        </ul>`
      : `<p class="detalhe-vazio">Nenhum veículo cadastrado ainda. Toque em “+ Veículo” para adicionar.</p>`;
    ...
    els.modalCorpo.innerHTML = `
      <div class="detalhe-cliente">
        ...
        <div class="detalhe-secao">
          <div class="detalhe-secao-cabecalho">
            <h3 class="detalhe-secao-titulo">Veículos vinculados (${veiculos.length})</h3>
            <button type="button" class="btn btn-secondary btn-sm" id="btn-add-veiculo">+ Veículo</button>
          </div>
          ${linhaVeiculos}
        </div>
    ...
    // Botão para cadastrar um novo veículo neste cliente.
    document.getElementById('btn-add-veiculo').addEventListener('click', () => {
      abrirFormularioVeiculo(cliente.id);
    });

    // Tocar em um veículo já cadastrado abre o formulário para editá-lo
    // (ou excluí-lo).
    els.modalCorpo.querySelectorAll('.detalhe-veiculo-item').forEach((item) => {
      const abrirEdicaoVeiculo = () => {
        const veiculo = veiculos.find((v) => v.id === Number(item.dataset.veiculoId));
        if (veiculo) abrirFormularioVeiculo(cliente.id, veiculo);
      };
      item.addEventListener('click', abrirEdicaoVeiculo);

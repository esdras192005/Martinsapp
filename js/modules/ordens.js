...
const OrdensModule = (() => {
  ...
  async function abrirFormulario(ordem = null) {
    ...
    els.modalCorpo.innerHTML = `
      <form id="os-form" novalidate>
        ...
        <div class="form-group">
          <label for="campo-os-descricao">Descrição detalhada dos serviços realizados *</label>
          <textarea id="campo-os-descricao" rows="4" maxlength="1000" placeholder="Ex: Troca de óleo e filtro, revisão do sistema de freios, diagnóstico de ruído na suspensão...">${escapeHtml(ordem?.descricaoServicos || '')}</textarea>
        </div>
    ...
    document.getElementById('os-form').addEventListener('submit', async (e) => {
      ...
      const descricaoServicos = document.getElementById('campo-os-descricao').value.trim();
      ...
      if (!descricaoServicos) {
        erroEl.textContent = 'Descreva os serviços realizados.';
        erroEl.hidden = false;
        return;
      }
    ...
    els.modalCorpo.innerHTML = `
      <div class="detalhe-os">
        ...
        <div class="detalhe-linha detalhe-linha-bloco">
          <span class="detalhe-rotulo">Descrição dos serviços</span>
          <span class="detalhe-valor">${ordem.descricaoServicos ? escapeHtml(ordem.descricaoServicos) : '—'}</span>
        </div>
  ...
  async function gerarPdfOrdem(ordem, cliente, veiculo) {
    ...
    const linhasPecas = ordem.pecasUtilizadas.length
      ? ordem.pecasUtilizadas.map((item) => `
          <tr>
            <td>${escapeHtml(item.descricao)}${(item.marca || item.codigo) ? `<br><small>${escapeHtml([item.marca, item.codigo].filter(Boolean).join(' · '))}</small>` : ''}</td>
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .assinaturas div { width: 45%; text-align: center; border-top: 1px solid #999; padding-top: 6px; font-size: 12px; color: #555; }
      ...
      <body>
        ...
        <div class="grid-info">
          ...
          <div><span>Telefone</span><strong>${escapeHtml(cliente?.telefone || '—')}</strong></div>
        ...
        <p class="descricao">${ordem.descricaoServicos ? escapeHtml(ordem.descricaoServicos) : 'Nenhuma descrição registrada.'}</p>
